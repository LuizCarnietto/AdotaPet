...
export default function EdicaoAnimal() {
  ...
  useEffect(() => {
    if (!id) {
      ...
      navigate("/cadastros/AnimaisCadastrados");
      return;
    }

    const obterFotos = (
      fotos: string | string[] | null | undefined,
    ): string[] => {
      if (!fotos) return [];
    ...
    const carregarFotos = (fotosRecebidas: any) => {
      const fotos = obterFotos(fotosRecebidas).slice(0, 3);
      ...
      fotos.forEach((foto, index) => {
        ...
        if (!img) {
          ...
          img.onclick = function () {
            ...
            document
              .getElementById("placeHolderImagem")
        ...
        if (index === 0) {
          ...
          if (imagemGrande) {
            imagemGrande.src = foto;
            imagemGrande.classList.add("ativa");
          }

          document.getElementById("placeHolderImagem")?.classList.add("oculto");
        }
      });
    };
    ...
    const buscarAnimal = async () => {
      try {
        ...
        setEspecieSelecionada(animal.especie || "");
        setRacaTexto(animal.raca || "");
        setRacaId(animal.racaId ? String(animal.racaId) : "");
        setCidadeId(animal.cidadeId ? String(animal.cidadeId) : "");

        // Descobre o estado pelo estadoSigla
        if (animal.estadoSigla) {
          const responseEstados = await apiService.get("/localizacao/estados");

          const estadoEncontrado = responseEstados.data.find(
            (estado: any) => estado.sigla === animal.estadoSigla,
          );
        ...
        // Campos que atualmente usam DOM/defaultValue
        setTimeout(() => {
          ...
          if (descricaoInput) {
            descricaoInput.value = animal.comportamento || "";
          }
          ...
          if (microchipInput) {
            microchipInput.value = animal.possuiChip ? "sim" : "nao";
          }

          if (sexoInput) {
            sexoInput.value =
              animal.sexo === "MACHO" ? "masculino" : "feminino";
          }
          ...
          if (corInput) {
            corInput.value = animal.cor || "";
          }

          if (vacinadoInput) {
            vacinadoInput.value = animal.vacinado ? "sim" : "nao";
          }

          carregarFotos(animal.fotos);
        }, 0);
      } catch (error) {
  ...
  const editarAnimal = async () => {
    try {
      ...
      console.log("Alterações enviadas:", payload);

      await apiService.put(`/animal/${id}`, payload);
    ...
    <>
      ...
      <div id="container">
        ...
        <div className="contDescricaoAnimal">
          ...
          <button id="botaoAzul" type="button" onClick={editarAnimal}>
            Salvar Edição
          </button>
        </div>
      </div>
